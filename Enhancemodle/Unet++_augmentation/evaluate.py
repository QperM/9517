...
from unet_plus_plus_augmentation_pipeline import UNetPlusPlus, compute_iou, get_test_augmentation, SegDataset
...
def measure_inference_time(model, test_loader, device, num_runs=10):
    ...
    avg_time_per_image = np.mean(times) / total_images
...
def main():
    ...
    print("Evaluating UNet++ Forest Segmentation Model with Data Augmentation...")
    ...
    test_dataset = SegDataset(test_imgs, test_masks, transform=test_transform, is_training=False)
    ...
    model_path = 'Enhancemodle/Unet++_augmentation/best_unet_plus_plus_model.pth'
    if os.path.exists(model_path):
        model.load_state_dict(torch.load(model_path, map_location=device))
        print("Best model weights loaded")
    else:
        # Try fallback to regular model
        model_path = 'Enhancemodle/Unet++_augmentation/unet_plus_plus_model.pth'
        if os.path.exists(model_path):
            model.load_state_dict(torch.load(model_path, map_location=device))
    ...
    if os.path.exists(model_path):
        model.load_state_dict(torch.load(model_path, map_location=device))
        print("Best model weights loaded for inference time measurement")
    else:
        model_path = 'Enhancemodle/Unet++_augmentation/unet_plus_plus_model.pth'
        if os.path.exists(model_path):
            model.load_state_dict(torch.load(model_path, map_location=device))
    ...
    with open('Enhancemodle/Unet++_augmentation/comprehensive_evaluation_results.txt', 'w', encoding='utf-8') as f:
        f.write("UNet++ Forest Segmentation Model with Data Augmentation - Comprehensive Evaluation Results\n")
        f.write("=" * 80 + "\n\n")
    ...
    print(f"Results saved to: Enhancemodle/Unet++_augmentation/comprehensive_evaluation_results.txt")
    ...
    test_imgs_txt = 'Enhancemodle/Unet++_augmentation/test_imgs.txt'
    test_imgs = []
