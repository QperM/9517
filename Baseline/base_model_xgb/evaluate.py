...
from xgb_pipeline import extract_features, compute_iou, compute_dice_coefficient, compute_pixel_accuracy, compute_classification_metrics

def measure_inference_time(model, test_data, num_runs=5):
...
def count_model_parameters(model):
    """Count total number of parameters in XGBoost model"""
    # For XGBoost, we count the number of trees and their parameters
...
def main():
    ...
    print("Evaluating XGBoost Forest Segmentation Model...")
    ...
    model_path = 'Baseline/base_model_xgb/xgb_model.pkl'
    ...
    for img in tqdm(X_test_imgs):
        ...
        pred_img = pred_proba.reshape(256, 256)
    ...
    if model is not None:
        # Load test images for inference time measurement
        test_imgs_txt = 'Baseline/base_model_xgb/test_imgs.txt'
        if os.path.exists(test_imgs_txt):
            with open(test_imgs_txt, 'r') as f:
                test_img_paths = [line.strip() for line in f.readlines()]
            
            # Load a few test images for timing
            test_imgs = []
            for img_path in test_img_paths[:5]:  # Use first 5 images for timing
                try:
            ...
            if test_imgs:
                try:
                    inference_time = measure_inference_time(model, test_imgs)
                    print(f"Inference time measured: {inference_time:.4f} seconds per image")
                except Exception as e:
    ...
    with open('Baseline/base_model_xgb/comprehensive_evaluation_results.txt', 'w', encoding='utf-8') as f:
        f.write("XGBoost Forest Segmentation Model - Comprehensive Evaluation Results\n")
        f.write("=" * 80 + "\n\n")
    ...
    print(f"Results saved to: Baseline/base_model_xgb/comprehensive_evaluation_results.txt")
    ...
    # 5. Enhanced Visualizations (just show, don't save)
    ...
    # Feature importance visualization (if model is available)
    if model is not None:
        feature_names = ['R', 'G', 'B', 'Gray', 'H', 'S', 'V', 'Gradient', 'Std_3x3',
                        'R_mean_5x5', 'R_std_5x5', 'G_mean_5x5', 'G_std_5x5', 'B_mean_5x5', 'B_std_5x5']
        importance = model.feature_importances_
        ...
        bars = plt.bar(range(len(sorted_names)), sorted_importance, color='skyblue', alpha=0.7)
        plt.xlabel('Features')
