...
def extract_balanced_features(image, mask):
    """Extract features with balanced sampling"""
    features_list = []
    labels_list = []
    
    # Find dead tree and background pixels
    dead_tree_coords = np.where(mask == 1)
    background_coords = np.where(mask == 0)
    
    # Sample more densely around dead tree areas
    step = 8  # More dense sampling
    
    # Sample from all pixels but ensure we get dead tree pixels
    for i in range(0, IMG_SIZE, step):
        for j in range(0, IMG_SIZE, step):
            # Get 16x16 patch around the pixel
            patch_size = 16
            ...
            features.extend([np.mean(gray_patch), np.std(gray_patch), np.var(gray_patch)])
    ...
    # Additional sampling from dead tree areas to balance the dataset
    if len(dead_tree_coords[0]) > 0:
        # Sample more dead tree pixels
        num_dead_tree_samples = min(1000, len(dead_tree_coords[0]))
        indices = np.random.choice(len(dead_tree_coords[0]), num_dead_tree_samples, replace=False)
        
        for idx in indices:
            i, j = dead_tree_coords[0][idx], dead_tree_coords[1][idx]
            ...
            features_list.append(features)
            labels_list.append(1)  # Dead tree label
...
def predict_image_svm_fixed(model, scaler, image):
    """Predict segmentation mask for an image using SVM - fixed version"""
...
def main():
    print("Starting Fixed SVM Forest Segmentation Pipeline...")
    ...
    for img_path, mask_path in tqdm(zip(train_imgs, train_masks), total=len(train_imgs)):
        ...
        # Extract features
        features, labels = extract_balanced_features(image, mask)
    ...
    for i in tqdm(range(len(X_test)), desc='Generating predictions'):
        pred_mask = predict_image_svm_fixed(svm_model, scaler, X_test[i])
